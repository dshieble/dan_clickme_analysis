...
def get_batch(img_ID_to_DATA, start, end, kind):
	X = img_ID_to_DATA["{}_data".format(kind)][start:end]/255.0
	y = img_ID_to_DATA["{}_labels".format(kind)][start:end]
	names = img_ID_to_DATA["{}_keys".format(kind)][start:end]
...
def get_adversarial_data():
    ...
    data_X = np.vstack([image_dict[fname][None,...] for fname in file_names])/255.0
...
# 	img_ID_to_DATA = np.load("{}/img_ID_to_DATA.h5py".format(data_path)).item()
